...
impl LibraryDownloadInfo {
    ...
    pub async fn download(
        ...
        if library_path.exists() {
            // If expected size is known, use it for a fast pre-check before SHA1
            if let Some(expected_size) = self.size {
                if let Ok(meta) = tokio::fs::metadata(&library_path).await {
                ...
                // No size known, trust the existing file
                progress.log(&format!("Library {} already exists.", name));
                return Ok(library_path);
            }

            // If sizes differ, re-verify with SHA1 to be safe
