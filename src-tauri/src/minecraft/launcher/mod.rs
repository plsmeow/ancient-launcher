...
pub async fn launch<D: Send + Sync>(
    ...
    launcher_data.progress_update(ProgressUpdate::set_label("Настройка..."));
    ...
    let game_dir = join_and_mkdir!(data, "gameDir");
    ...
    for x in command_arguments.iter() {
        mapped.push(process_templates(x, |output, param| {
            match param {
                ...
                "launcher_name" => output.push_str("AncientLauncher"),
                ...
                "clientid" => output.push_str(&launching_parameter.auth_uuid),
